 ...
 *
 * @author Henrik Drefs
 * @see com.dexmohq.imadex.auth.crypto.KeyProvider
 * @see com.dexmohq.imadex.auth.crypto.JceksKeyProvider
 * @see com.dexmohq.imadex.auth.crypto.PasswordKeyProvider
 * @see BCrypt
 */
public class Aes256BCryptSha512PasswordEncoder implements PasswordEncoder {
    ...
    private byte[] randomIv(int size) throws NoSuchAlgorithmException {
        ...
        final SecureRandom random = SecureRandom.getInstance("SHA1PRNG");// todo or getInstanceStrong()
        random.nextBytes(iv);
        return iv;
    }
